/**
 * Check whether the browser profile is signed in to LinkedIn, and if not, park
 * Chrome on the sign-in page so you can log in by hand.
 *
 *   npm run login
 *
 ...
 * across runs, so the demo does not start with a login and a 2FA code on
 * camera.
 *
...
import { LOGIN_CHECK_PROMPT } from '../linkedin.mjs';
...
console.log('Checking LinkedIn sign-in state ...\n');
const turn = await client.runTurn(sessionId, [
  { type: 'user.message', content: LOGIN_CHECK_PROMPT },
]);
...
if (answer.startsWith('LOGGED_IN')) {
  console.log('✓ Signed in. The profile is ready — nothing to do.');
  process.exit(0);
}

if (answer.startsWith('CHALLENGE')) {
  console.error(`\n✗ LinkedIn is showing a challenge, not the feed:\n\n    ${answer}\n`);
  console.error(
...
console.log('Chrome is open on LinkedIn with the profile this project uses.');
